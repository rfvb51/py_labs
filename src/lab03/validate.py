...
def _validate_has_wifi(value):
    if not isinstance(value, bool):
        raise TypeError("наличие Wi-Fi должно быть логическим значением")
    return value

def _validate_has_ac(value):
    if not isinstance(value, bool):
        raise TypeError("наличие кондиционера должно быть логическим значением")
    return value

def _validate_route_type(value):
    allowed = ["городской", "пригородный", "междугородний"]
    if value not in allowed:
        raise ValueError(f"тип маршрута должен быть одним из: {allowed}")
    return value

def _validate_has_supervisor(value):
    if not isinstance(value, bool):
        raise TypeError("наличие сопровождающего должно быть логическим значением")
    return value